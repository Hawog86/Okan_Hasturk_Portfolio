...
OR `Area Name` LIKE '%Ashland Town%' OR `Area Name` LIKE '%Fremont Town%'
;
...
# have to be identified first. REGULAR EXPRESSIONS and METACHARACTERS are going to be used for this queryfor identyfing.
...
SELECT `Area Name`, CONCAT(SUBSTRING_INDEX(`Area Name`,'town',1),' ',RIGHT(`Area Name`,4))
FROM census
WHERE `Area Name` REGEXP 'town$'
;
...
# At this stage of the Data Cleaning and Standardization phase, the separation of the...
# ... names have to be utilized. First they need to be determined. They are only to be...
# ... found within the names that ends 'town'.

SELECT `Area Name`, SUBSTRING_INDEX(`Area Name`,'town',1)
FROM census
WHERE `Area Name` REGEXP 'town$'
;

# There are some names like ShelterIsland, EastHampton. They are to be separated from the second...
# ... capital letter and a blank is going to be applied between them. For that the first capital letter...
# ... has to be removed

SELECT SUBSTRING_INDEX(SUBSTRING(`Area Name`,2),1,-1)
FROM census
WHERE `Area Name` REGEXP '[A-Z]'
;
...
SELECT `Population Percent Change`, (`2020 Census Population` - `2010 Census Population`) / `2010 Census Population`
FROM census
;

# As it can be determined above, both columns match and there aren't any rows that doesn't match.
